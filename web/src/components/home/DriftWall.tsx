...
export default function DriftWall({
  ...
  const renderTile = (item: DriftWallItem, id: string, colIndex: number) => {
    const inner = (
      <span className={styles.driftWallInner}>
        <img src={item.image} alt={item.title ?? ''} loading="lazy" decoding="async" draggable={false} />
        <span className={styles.driftWallOverlay} aria-hidden="true" />
      </span>
    )
    const isActive = activeId === id
    const tileClass = `${styles.driftWallTile}${isActive ? ` ${styles.isActive}` : ''}`
    if (item.href) {
      return (
        <a
          key={id}
  ...
  const rootClass = [styles.driftWall, reduced ? styles.driftWallReduced : '', className].filter(Boolean).join(' ')
  ...
  return (
    ...
    >
      <div ref={planeRef} className={styles.driftWallPlane}>
        {columnItems.map((col, c) => {
          const meta = columnMeta[c]
          const copies = Array.from({ length: meta.copies })
          return (
            <div className={styles.driftWallCol} key={`col-${c}`}>
              <div
                className={styles.driftWallTrack}
                ref={(el) => {
