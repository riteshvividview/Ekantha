...
class MorphEngine {
  ...
  goTo(dir: number) {
    if (this.animating || this.dragging || this.items.length < 2 || dir === 0) return
    const opts = this.getOptions()
    if (!opts.loop) {
...
>(function MorphSlider(
  ...
  // `startIndex` seeds the engine once on mount only — it must NOT be a
  // dependency below, or every externally-driven index change (arrow click,
  // dot click) would tear down and recreate the WebGL engine mid-morph,
  // killing the transition before it can animate.
  ...
  useImperativeHandle(
    ...
    () => ({
      ...
      goTo: (i: number) => {
        ...
        const forward = engine.wrap(engine.current + 1) === target ? 1 : -1
        const dir = n <= 2 ? forward : target - engine.current
        engine.goTo(dir)
      },
    }),
    [items.length],
  ...
  return (
    ...
    >
      <div ref={containerRef} className={styles.stage} role="group" aria-roledescription="carousel" aria-label="Stay image slider" />
    </div>
  )
})
